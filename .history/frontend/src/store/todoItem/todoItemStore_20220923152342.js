...
const getters = {
  ...
  totalTodo: (state) => state.todoList.filter((todoList) => todoList.done), // todolist를 순회하면서 false 개수 셈 -> 총 개수
  totalTodoCount: (state, getters) => getters.totalTodo.length,
  doneTodo: (state) => state.todoList.filter((todoList) => !todoList.done),
  doneTodoCount: (state, getters) => getters.todoList.length,
};
const actions = {
  /**
   * create
